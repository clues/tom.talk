 ...
 */
    ...
    var processSignalingMessage = function(message) {
        var msg = JSON.parse(message.replace(/\r\n/g, "\\r\\n"));

        if (msg.type === 'chat' || msg.type === 'get_room_ok' || msg.type=='inout') {
            if (null === callbackReceiveChatMessage) return;
            callbackReceiveChatMessage(msg);
        } else if (msg.type === 'offer') {
        	var desp={};
        	desp.type= 'offer';
        	desp.sdp = msg.value.replace(/\\r\\n/g, "\r\n");
            pc.setRemoteDescription(new RTCSessionDescription(desp));
            doAnswer();
            ...
            var candidate = new RTCIceCandidate({sdpMLineIndex:parseInt(msg.label), candidate:msg.candidate.replace(/\\r\\n/g, "\r\n")});
    ...
    this.chatSendMessage = function(chatMsg) {
        ...
        sendMessage({"type" : "chat", "value" : chatMsg});
    };
